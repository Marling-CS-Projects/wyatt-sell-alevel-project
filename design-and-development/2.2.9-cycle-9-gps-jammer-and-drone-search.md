...
![](../.gitbook/assets/MOSHED-2022-9-15-14-52-57.gif)![](<../.gitbook/assets/image (1) (6).png>)
...
<figure><img src="../.gitbook/assets/image (4) (4).png" alt=""><figcaption><p>A flowchart to show what happens when a player uses an item</p></figcaption></figure>
...
'item-active': data => {
  if (me?.items) {
    ...
    // Updates the matching item within the client players item array,
    // setting the active and activeStart parameters.
    setPlayers(prev => [
...
export const GridItem = (props: {item: Item | undefined}) => {
  ...
  useEffect(() => {
    if (item?.activeStart) {
      // Creates an interval function that runs every second, and updates
      // the displayed time.
      const interval = setInterval(() => {
        // Calculates the newTime by subtracting the seconds elapsed since the
        // item was activated from the duration of the item's effect
        const newTime =
...
'item-used': data => {
  if (me?.items) {
    // Removes item from players array
    setPlayers(prev => [
      ...prev.filter(p => p.id !== me.id),
