...
export const WinLose = () => {
  ...
  useEffect(() => {
    // Updates the coundown state every second to produce a 10 second countdown
    const interval = setInterval(() => {
      if (countdown === 0) {
        // Once the countdown reaches zero, the client disconnects and 
        // leaves the game.
        socket?.disconnect();
        setGame(null);
        setSocket(null);
  ...
  // Ensures that the required attributes are not undefined
  if (!game || !game.hasEnded || !game.winningTeam) return null;

  return (
    // Uses a reusable overlay component that fills the whole screen.
    <Overlay bg={game.winningTeam === 'hunted' ? 'rgba(218,81,81,0.8)' : 'rgba(81,127,218,0.8)'}>
      <Text fontSize={'5vw'} color={'white'}>
        Congratulations
      </Text>
      <Heading fontSize={'15vw'} color={'white'}>
        {game.winningTeam.toUpperCase()} WIN
      </Heading>
