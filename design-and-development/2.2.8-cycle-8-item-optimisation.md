...
On receipt of the `item-pickup` message, several processes need to occur (see [#pseudocode](2.2.8-cycle-8-item-optimisation.md#pseudocode "mention")). We need to get the item by its id, append it to the player object, and remove it from the game object, before sending the updated data to the client.
