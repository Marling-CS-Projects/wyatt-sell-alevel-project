...
**Semantics**: Currently, there are several names of properties that are confusing and have caused frequent issues:

* `hunter` **and** `hunted` **** - The one letter difference means that typos can cause significant issues. in order to fix this, I intend to rename hunter and hunted (internally!), to `catcher` and `hider`. No change will be made to how the names are displayed to the players, given the TV show theme, but this will allow for more readable and less error-prone development. Additionally, I intend to replace the `type` string value on each player with an `isHider` boolean value to simplify player type checking.
* \`catcher
